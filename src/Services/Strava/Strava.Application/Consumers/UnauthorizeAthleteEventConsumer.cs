...
public sealed class UnauthorizeAthleteEventConsumer : IConsumer<UnauthorizeAthleteEvent>
{
    ...
    public async Task Consume(ConsumeContext<UnauthorizeAthleteEvent> context)
    {
        ...
        if (token is not null)
        {
            ...
            _logger.LogInformation("Athlete:{AthletId} token has been removed successfully.", context.Message.StravaUserId);

            return;
        }

        _logger.LogInformation("Athlete:{AthleteId} token not found.", context.Message.StravaUserId);
    }
}
